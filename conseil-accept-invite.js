...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ── GARDE-FOU : interdiction du double rôle ──────────────────────
    // Un compte ne peut pas être à la fois client Alteore ET conseiller.
    // Les deux rôles partagent le MÊME Firebase Auth (même domaine, même
    // session) ; les laisser cohabiter sur un seul UID fait se mélanger
    // les données du conseiller et celles du client qu'il consulte.
    //
    // Signal fiable : un doc `users/{uid}` n'existe QUE pour un compte
    // client (créé par l'onboarding de l'app principale). Un conseiller
    // « pur » (créé via accept-invite) n'en a jamais. Donc si ce doc
    // existe, le compte est déjà un client → on refuse de le transformer
    // en conseiller. Idem si le conseiller = le client (auto-grant).
    if (conseillerUid === clientUid) {
      return res.status(409).json({
        error: 'Vous ne pouvez pas vous accorder un accès conseiller sur votre propre compte.'
      });
    }
    const existingUser = await fsGet(`users/${conseillerUid}`, adminToken);
    if (existingUser && existingUser.name) {
      return res.status(409).json({
        error: 'Ce compte est déjà un compte client Alteore. Pour consulter un client en tant que conseiller, créez un compte séparé avec une autre adresse email (différente de celle de votre compte Alteore).'
      });
    }
