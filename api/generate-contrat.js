...
2. MENTIONS OBLIGATOIRES : Chaque contrat DOIT inclure TOUTES les mentions légales obligatoires selon son type. Ne jamais en omettre.
...
4. FORMAT : Le contrat doit être en HTML propre et structuré :
   - Utilise <h1> pour le titre du contrat
   - <h2> pour les articles
   - <p> pour les paragraphes
   - <strong> pour les mentions importantes
   - <ul>/<li> pour les listes
   - Pas de CSS inline complexe — le rendu sera géré côté client
   - Commence DIRECTEMENT par le HTML du contrat, sans texte d'introduction ni balise markdown
...
6. Pour les CDD obligatoirement ajouter : motif de recours, nom du salarié remplacé si remplacement, date de fin ou durée minimale, possibilité de renouvellement.
...
export default async function handler(req, res) {
  ...
  if (!auth) return;
  if (_rateLimit(auth.uid, res, 8)) return;
