// api/send-sms.js
// Envoie une campagne SMS via Capitole Mobile + débite le solde Firebase

// Auth Firebase (même fonction que stripe-webhook)
...
// Envoyer les SMS via Capitole Mobile (API XML)
async function sendViaCaptioleMobile(recipients, message, sender) {
  ...
  // Construire la liste des destinataires XML
  const gsmTags = recipients
    .map(tel => {
      // Normaliser le numéro : 0612... → 33612...
  ...
  // Tronquer le sender à 11 caractères max (limite Capitole Mobile)
  ...
  // Capitole Mobile renvoie un XML — on vérifie s'il y a une erreur
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // 1. Vérifier le solde SMS
    const creditsDoc = await firestoreGet(token, projectId, `sms_credits/${uid}`);
    const currentCredits = parseInt(
      creditsDoc?.fields?.credits?.integerValue || '0'
    );
    ...
    // 2. Envoyer les SMS via Capitole Mobile
    const capitoleResponse = await sendViaCaptioleMobile(recipients, message, sender);

    // 3. Débiter le solde
    const newCredits = currentCredits - smsNeeded;
    await firestorePatch(token, projectId, `sms_credits/${uid}`, {
      credits: newCredits,
    });

    // 4. Enregistrer la campagne dans l'historique
    ...
    console.log(`✅ ${smsNeeded} SMS envoyés — uid ${uid} — solde restant: ${newCredits}`);
